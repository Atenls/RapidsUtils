...
- Completed topics are expired on client ticks even when the HUD is disabled, hidden with F1, or cannot fit on screen. A new snapshot arriving after the old fade-out completes also starts a new fade-in before the next cleanup tick. Updates during an unfinished fade-out retain the original fade-in start. Client fallback durations are read from the current topic configuration for both rendering and expiry.
...
The first valid message activates the player-vitals override. While active, the vanilla health row is suppressed and the health and mana bars share one 182-pixel-wide row in the vanilla health and hunger row position. Each bar occupies roughly 40% of that row: health is aligned left and mana is aligned right.

Five styles are built in and can be selected from the Mod Menu settings screen or with `vitalsBarStyle` in `config/rapidsutils.json`: `A` is a nine-pixel misted-glass bar and is the default, `B` adds a one-pixel hairline frame, `C` removes the complete frame and floats the liquid over a narrow track, `D` restores the compact rounded flat-color style, and `E` adds slim silver end caps and a center etching. Health linearly interpolates from dark blood red at zero to bright red at full health; mana remains blue. All five styles keep the centered value text. At 25% health or below, the health bar occasionally performs a short pixel shake and drops a small blood pixel from the current fill edge.

Vitals values use a dedicated smooth renderer with [Roboto Mono](https://github.com/googlefonts/RobotoMono) Medium (weight 500), subset to printable ASCII as **Rapids Vitals**. The font and derived atlases remain under SIL OFL 1.1; see the bundled `assets/rapidsutils/font/ofl_roboto_mono.txt`. Digits and letters use six-pixel monospaced advances, with a compact 2.5-pixel decimal point, and approximately seven GUI pixels of visible height. All five styles draw values without a text shadow and center the cap area; style C uses its full nine-pixel HUD area.

The renderer bypasses Minecraft's nearest-sampled font atlas. A retained 16x master (112-pixel cap height) supplies area-filtered atlases for GUI scales 1 through 8; higher scales use the 16x atlas. Glyphs are drawn at HUD size using linear texture sampling and coverage alpha, with origins aligned to physical pixels. Transparent texels retain white RGB to prevent dark fringes. The same metrics apply at every density, so changing GUI scale does not alter the bar layout. This improves small text antialiasing; GUI scale 1 still has only seven physical pixels of cap height. Other HUD text and the Force Unicode Font option are unaffected. No extra resource pack or setting is required. Texture resources reload normally with F3+T; layout metrics are bundled with the mod.

Rebuild the static font and atlases with `python tools/generate_vitals_font.py path/to/RobotoMono-variable.ttf` (fonttools 4.64.0 and Pillow 12.3.0). The script verifies the reviewed upstream variable-font revision and produces the bundled static TTF plus atlases; the HUD uses the prefiltered textures rather than the vanilla font provider.
