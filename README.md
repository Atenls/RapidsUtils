...
Vitals values use the bundled `rapidsutils:vitals` TrueType font independently of the Force Unicode Font option. Its rounded outlines come from [Nunito](https://github.com/googlefonts/nunito), instantiated at SemiBold weight 600 and subset to printable ASCII as **Rapids Vitals**. The font and derivative are licensed under SIL OFL 1.1; the full license is bundled beside the font in `assets/rapidsutils/font/ofl_nunito.txt`. At size 10, digits have approximately seven-pixel visible height and six-pixel tabular advances. An oversample factor of 8 rasterizes curves at higher resolution before displaying them at HUD size. The nine- and eleven-pixel styles center the seven-pixel cap area, including the shadow where enabled; style C uses the full nine-pixel HUD area. GUI scale applies normally. Other HUD text continues to use its existing font. No extra resource pack or configuration is required. Rebuild the static subset with `python tools/generate_vitals_font.py path/to/Nunito-variable.ttf` (fonttools 4.64.0); the script pins and checks the upstream font revision. Custom resource packs can deliberately override this named font.
