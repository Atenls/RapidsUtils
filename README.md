...
Vitals values use the bundled `rapidsutils:vitals` bitmap font independently of the Force Unicode Font option. It contains original seven-pixel-high ASCII glyphs, equal-width digits, and compact punctuation. The nine- and eleven-pixel styles keep the font at its native size and center its visible bounds, including the shadow where enabled; style C uses the full nine-pixel HUD area. GUI scale applies normally. Other HUD text continues to use its existing font. No extra resource pack or configuration is required. The glyph source is `tools/GenerateVitalsFont.java`; regenerate the committed PNG from the repository root with `java tools/GenerateVitalsFont.java` (JDK 21). Custom resource packs can deliberately override this named font.
